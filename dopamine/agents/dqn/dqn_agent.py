...
class DQNAgent(object):
	...
	def _build_train_op(self):
		...
		replay_chosen_q = tf.reduce_sum(
				self._replay_net_outputs.q_values * replay_action_one_hot, axis=1, name='replay_chosen_q')
	...
	def step(self, reward, observation, **kwargs):
		...
		We store the observation of the last time step since we want to store it with the reward.
		...
		if not self.eval_mode:      # 存储样本. 调用子类的 _train_step 函数
