...
class MMDAgent(dqn_agent.DQNAgent):
	...
	def _build_target_particles(self):
		# 先计算 r+gamma*(1-done)*Q(s', for all action). 然后调用 compute_target 根据选择来返回. (bs,n)
	...
	def _build_networks(self):
		...
		self._action_sampler.draw_action(self._net_outputs.particles, self._policy, beta=self._beta_ph)[0]  # _policy 默认 eps_greedy
	...
	def _build_train_op(self):  # *Required.
		...
		if self.bandwidth_selection_type == 'mixture':  # mixture of kernels from MMD GAN
			...
			if self.bandwidth_selection_type == 'med':  # Use median trick
				...
				med_sq = tfp.stats.percentile(tf.reshape(aggregated_d, [batch_size, -1]), q=50., axis=1,
											  interpolation='midpoint')  # (bs,) 相当于每行的元素取了中位数
...
class ParticlePolicy(object):
	...
	def compute_thompson_matrix(particles):
		...
		logits = []
		for i in range(na):
			q1 = particles[:, i, :]                      # 采样得到的 (bs, n), 对应于某个 action
			i_index = tf.constant(np.array([j for j in range(na) if j != i]))
			q2 = tf.gather(particles, i_index, axis=1)   # 对应于其它所有 action 的 gather 结果 (bs, na-1, n)
			s = tf.cast(tf.greater_equal(q1[:, None, None, :], q2[:, :, :, None]), dtype=tf.float32)  # (bs, na-1, n, n)  检测大于等于
	...
	def sample_from_action_probability(action_values):
		...
		"""
		action_prob = tf.cast(tf.equal(tf.reduce_max(action_values, axis=1)[:, None], action_values), tf.float32)   # 每行的 max 所在位置
		...
		return selected_action  # tf.squeeze(selected_action)    # 选择每行 action-value 值最大的动作
	...
	def draw_action(self, particles, policy='mean', head_index=0, random_weights=np.array([1]), beta=None):
		...
		if policy in ['eps_greedy', 'mean']:
			...
			# A head is sampled at each time step, as opposed to bootstrapped policy where a head is sampled at each episode.
	...
	def compute_target(self, targets, estimator='mean', random_weights=np.array([1])):
		...
		if estimator == 'mean':
			...
			return action_targets    # (bs, n). 其中 action 维度被去掉，选择了最好的 action
