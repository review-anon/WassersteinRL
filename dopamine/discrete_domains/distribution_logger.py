...
class DistributionLogger(object):
	...
	def log_evaluation(self, prediction_samples, mc_samples, episode_idx, num_steps=None):
		# 用来记录 policy evaluation 的日志，包括绘图.
		# prediction_samples 是网络输出的对初始状态 S0 的评价, 维度是 (None, reward_dim).
		# mc_samples 是使用 monte carlo 评价得到的样本, 维度是 (None, reward_dim)
		...
		# 将 policy evaluation 的样本和 MMD 距离等存入文件
		data = {
		...
		# 绘制散点图. prediction_samples 和 mc_samples 样本越接近越好
		reward_dim = prediction_samples.shape[1]
		# plot 2d joint distribution
		for d1 in range(reward_dim):
			for d2 in range(d1 + 1, reward_dim):
				prediction_d1, prediction_d2 = prediction_samples[:, d1], prediction_samples[:, d2]
				plt.scatter(prediction_d1, prediction_d2, alpha=0.5, zorder=1)

				mc_d1, mc_d2 = mc_samples[:, d1], mc_samples[:, d2]
				plt.scatter(mc_d1, mc_d2, alpha=0.5, zorder=2)

				plt.legend(['prediction', 'monte-carlo'])

				plt.savefig(os.path.join(path, f'scatter-dim{d1}-dim{d2}-episode-{episode_idx}.pdf'))
				plt.clf()
	...
	def log(self, samples, next_samples, states, actions, rewards, next_states, losses, gamma):
		...
		for idx in range(batch_size):
			...
			# plot reward samples.  samples: Q(s,a)   next_samples: Q(s',a'). shape = (bs, sample_num, reward_dim)
			...
			for d in range(reward_dim):
				# 某一个 reward 维度的分布. sample.shape=(sample_num, reward_dim).   sample_d.shape = (sample_num).
				sample_d = sample[:, d]
