...
from dopamine.agents.gan_nd import gan_nd_agent
...
def create_agent(sess, environment, agent_name=None, summary_writer=None,
	...
	#                             summary_writer=summary_writer)
		...
		return mmdqn_nd_agent.MMDAgent(sess, num_actions=environment.action_space.n,
		                               summary_writer=summary_writer)
	elif agent_name == 'gan_nd':
		return gan_nd_agent.GANAgent(sess, num_actions=environment.action_space.n,
		                             summary_writer=summary_writer)
...
class Runner(object):
	...
	def __init__(self,
		...
		print("\n Evaluation Setting: ", self.evaluation_setting)
	...
	def _initialize_checkpointer_and_maybe_resume(self, checkpoint_file_prefix):
		...
		self._checkpointer = checkpointer.Checkpointer(self._checkpoint_dir,
		                checkpoint_file_prefix, checkpoint_frequency=1)
		...
		if latest_checkpoint_version >= 0:
			...
			if self._agent.unbundle(
				...
				logging.info('Reloaded checkpoint and will start from iteration %d', self._start_iteration)
	...
	def _run_one_episode(self):
		...
		"""
		constrain_env = ('constrain_env' in dir(self._environment) and self._environment.constrain_env)  # 一般是 false
		...
		if constrain_env:
			...
			initial_kwargs = {'remained_constraint': initial_constraint, 'is_less_constraint': is_less_constraint}
		...
		while True:
			...
			elif type(reward) is np.ndarray:      # 进入. reward 是 reward_dim 维度的 npy
			...
			if (self._environment.game_over or
				...
				# 在 policy evaluation 中实际输出的是 random action. 因为默认评价的是 random policy
				# _agent.step 中会保存样本到经验池 (在 train 状态下)
				# _agent.step 会根据现有经验池样本进行一次训练 (在 train 状态下)
				# if self._agent._replay.memory.add_count > self._agent.min_replay_history:
				# 	print("\nIn _run_one_episode. 开始")
				action = self._agent.step(reward, observation, **kwargs)
				# if self._agent._replay.memory.add_count > self._agent.min_replay_history:
				# 	print("In _run_one_episode. 结束\n")
	...
	def _run_one_phase(self, min_steps, statistics, run_mode_str):
		...
		if self._agent.eval_policy is None:     # 默认
			# eval_after_steps > 0 is not supported for the maze case where "eval_policy" is None
			eval_after_steps = 0                # 默认

		if self.evaluation_setting and evaluate_in_this_iteration:   # maze 时为 true
			# print("进入:", self.evaluation_setting, run_mode_str)
			if eval_after_steps == 0:
				# 对初始状态进程 monte carlo 的 policy evaluation. 使用的策略是随机策略
				obs = self._environment_test.reset()
				# 进行 monte carlo policy evaluation. 维度 (None, reward_dim)
		...
		while step_count < min_steps:         # policy evaluation 进入. min_steps=250000
			if self._agent.maze_env:
				log_per_episodes = 500
			else:
				log_per_episodes = 10
			if self.evaluation_setting and evaluate_in_this_iteration and (num_episodes % log_per_episodes == 0):  # log_per_episodes = 500
				if eval_after_steps == 0:
					# 对初始状态, 返回实际网络预测的输出 (sample_num, reward_dim)
				...
				print(f'prediction {particles.shape}: mean={particles.mean()}, std={particles.std()}, max={particles.max()}, min={particles.min()}')
				print(f'mc_samples {mc_samples.shape}: mean={mc_samples.mean()}, std={mc_samples.std()}, max={mc_samples.max()}, min={mc_samples.min()}')
				print("---------------------- Eval:", self._agent.eval_mode)
				total_train_steps = step_count + distribution_logger.iteration * self._training_steps
				# 输入 网络的预测particles 和 mc真实的mc_samples, 计算二者的 mmd loss 并绘制样本分布的散点图
				distribution_logger.log_evaluation(particles, mc_samples, num_episodes, num_steps=total_train_steps)

			# 调用 run_one_episode 函数. 该函数用待评价策略进行一个周期的交互, 返回交互结果. 并进行训练
			episode_length, episode_return = self._run_one_episode()
			statistics.append({'{}_episode_lengths'.format(run_mode_str): episode_length,
			...
			# We use sys.stdout.write instead of logging so as to flush frequently without generating a line break.
	...
	def _run_train_phase(self, statistics):
		"""Run training phase.   在 policy evaluation 过程中每隔一段时间来进行一次训练
		Args:
		  statistics: `IterationStatistics` object which records the experimental
	...
	def _run_eval_phase(self, statistics):
		...
		# 仅交互. 记录 returns, episodes. 记录 monte carlo 评价的结果和 网络的预测结果
		_, sum_returns, num_episodes = self._run_one_phase(
			self._evaluation_steps, statistics, 'eval')
		average_return = sum_returns / num_episodes if num_episodes > 0 else 0.0
		logging.info('Average undiscounted return per evaluation episode: %.2f', average_return)
		statistics.append({'eval_average_return': average_return})
		wandb.log({"evaluation return": average_return})
	...
	def _run_one_iteration(self, iteration):
		...
		# 交互多个周期. 返回交互的周期数目和周期的平均 return. 如果不输入交互的策略, 那么默认使用随机策略交互.
	...
	def run_experiment(self):
		...
		# policy evaluation 下默认是 _num_iterations = 20
